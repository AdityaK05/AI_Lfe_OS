...
def get_progress(db: Session=Depends(get_db), request: Request=None):
    ...
    for r in routines:
        log = db.query(models_db.HabitLog).filter(models_db.HabitLog.routine_id == r.id).order_by(models_db.HabitLog.completed_at.desc()).first()
        if log and log.completed_at.date() == today:
            routines_completed += 1
    return schemas_productivity.ProgressStats(tasks_completed_today=tasks_done, tasks_total_today=tasks_total, routines_completed_today=routines_completed, routines_total_today=routines_total)