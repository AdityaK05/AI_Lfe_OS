...
def create_task(title: str, description: str = None, due_date: str = None) -> str:
    """Create a new task in the user's productivity system.
    Returns the created task ID or an error message.
    """
    db: Session = SessionLocal()
    try:
        # We assume user_id is passed implicitly via state or we just use default for the agent
        # In a real setup, you'd pass user_id via config to the tool
...
@tool
def list_tasks() -> str:
    """List all the user's pending tasks."""
    db: Session = SessionLocal()
    try:
        user_id = "default_user"
...
def create_routine(title: str, frequency: str = "daily", time_of_day: str = None) -> str:
    """Create a new routine or habit for the user to track."""
    db: Session = SessionLocal()
    try:
        user_id = "default_user"
