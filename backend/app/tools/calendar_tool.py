...
def list_events(days: int=7) -> list[dict]:
    try:
        ...
        events_result = service.events().list(calendarId='primary', timeMin=now, timeMax=end, maxResults=50, singleEvents=True, orderBy='startTime').execute()
        events = events_result.get('items', [])
        structured_events = []
        for event in events:
            start = event['start'].get('dateTime', event['start'].get('date'))
            end_t = event['end'].get('dateTime', event['end'].get('date'))
            structured_events.append({'id': event['id'], 'summary': event.get('summary', 'Untitled'), 'start': start, 'end': end_t})
...
def create_event(title: str, start: str, end: str, description: str='') -> dict:
    try:
        service = _get_calendar_service()
        event = {'summary': title, 'description': description, 'start': {'dateTime': start, 'timeZone': 'UTC'}, 'end': {'dateTime': end, 'timeZone': 'UTC'}}
